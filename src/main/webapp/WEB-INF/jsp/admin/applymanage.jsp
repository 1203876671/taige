...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
				<a class="btn btn-primary radius" href="upapply.html?id=0"><i class="Hui-iconfont">&#xe600;</i> 添加分类</a>
			</span>
            ...
            <c:forEach items="${pageInfo.list }" var="item">
                <tr class="text-c">
                    ...
                    <td class="f-14 td-manage">
                        <a style="text-decoration:none" class="ml-5" href="upapply.html?id=${item.id }" title="编辑"><i
                                class="Hui-iconfont">&#xe6df;</i></a>
                        <a href="javascript:del(${item.id })"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        <a style="text-decoration:none" class="ml-5" href="applydetailsmanage.html?applyid=${item.id }"
    ...
    <div id="PageNum">
        <section>
            <div class="bd points-goods-list">
                <ul class="pages">
                    <li><a href="applymanage.html?page=1&rows=${pageInfo.pageSize}">首页</a></li>
                    <li class="prev"><a
                            href="applymanage.html?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}">上一页</a>
                    </li>
                    <c:forEach items="${pageInfo.navigatepageNums}" var="nav">
                        <c:if test="${nav == pageInfo.pageNum}">
                            <li><a href="applymanage.html?page=${nav}&rows=${pageInfo.pageSize}"
                                   class="active">${nav}</a></li>
                        </c:if>
                        <c:if test="${nav != pageInfo.pageNum}">
                            <li><a href="applymanage.html?page=${nav}&rows=${pageInfo.pageSize}">${nav}</a></li>
                        </c:if>
                    </c:forEach>
                    <li class="next"><a
                            href="applymanage.html?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}">下一页</a>
                    </li>
                    <li><a href="applymanage.html?page=${pageInfo.pages }&rows=${pageInfo.pageSize}">末页</a></li>
                </ul>
            </div>
        </section>
    </div>
...
<script type="text/javascript">
    ...
    function del(id) {
        layer.confirm('确认要删除应用及其子内容吗？', function (index) {
            $.post("deleteapply.html", {id: id}, function (data) {
                if (data.is) {
