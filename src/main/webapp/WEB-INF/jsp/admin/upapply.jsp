...
<nav class="breadcrumb">
    <a><i class="Hui-iconfont">&#xe67f;</i> ； 应用类别管理</a>
    <span class="c-gray en">&gt;</span> 修改应用类别信息
    <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
       href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a>
</nav>
<h3 style="text-align:center;">应用类别修改/添加</h3>
<article class="page-container">
    <form class="form form-horizontal" id="form-article-add" enctype="multipart/form-data"
          action="${pageContext.request.contextPath}/admin/insertapply.html" method="post">
        <input name="id" hidden="hidden" value="${apply.id}"/>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-1"><span class="c-red">*</span>应用名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${apply.name}" placeholder="" id="name" name="name">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-1"><span class="c-red">*</span>应用简介：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${apply.title}" placeholder="" id="title" name="title">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-1">排序值：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <c:if test="${apply.sort != null && apply.sort != ''}">
                    <input type="text" class="input-text" value="${apply.sort}" placeholder="0" id="sort" name="sort">
                </c:if>
                <c:if test="${apply.sort == null || apply.sort == ''}">
