...
<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> 修改招聘信息
    <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
       href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a>
</nav>
<h3 style="text-align:center;">招聘信息修改/添加</h3>
<article class="page-container">
    <form class="form form-horizontal" id="form-article-add" enctype="multipart/form-data"
          action="${pageContext.request.contextPath}/admin/insertinvite.html" method="post">
        <input name="id" hidden="hidden" value="${invite.id}"/>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-1"><span class="c-red">*</span>招聘名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="${invite.name}" placeholder="" id="name" name="name">
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <select class="input-text" name="state">
                    <c:if test="${invite.state == 1}">
                        <option value="1" selected>显示</option>
                    </c:if>
                    <c:if test="${invite.state != 1}">
                        <option value="1">显示</option>
                    </c:if>
                    <c:if test="${invite.state == 2}">
